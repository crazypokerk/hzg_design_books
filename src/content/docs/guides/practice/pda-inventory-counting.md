---
title: PDA 盘点扫码页设计演示
description: 用一个仓库盘点案例演示如何把 PDA 连续扫码页从需求拆解到页面结构、命令编排、异常恢复和真机验收。
sidebar:
  order: 2
---
...
这不是一个完整业务系统方案，而是一份可以照着练习和评审的页面设计演示。读完后，应能产出一份足够交给实施或开发的 `PDA` 盘点扫码页说明。
...
这个页面的目标不是展示完整库存台账，而是让盘点员在货架前快速完成一条又一条盘点记录。完整历史、统计报表、操作日志都应该后置。

## 现场约束

设计前先把真实现场讲清楚。盘点现场通常不是安静办公室，页面要默认面对这些约束：

| 约束 | 对页面的影响 |
| --- | --- |
| 单手或双手持 PDA | 主操作要固定，触控目标要大 |
| 连续扫码 | 成功后自动准备下一条，不反复返回列表 |
| 扫描头可能带回车 | 回车行为必须稳定，不能误提交 |
| 仓库弱网 | 支持本地暂存、待补传数量和重试 |
| 货架前光线复杂 | 状态文字要清楚，不能只靠颜色 |
| 物料和库位容易扫错 | 失败后保留码值并说明原因 |
| 盘点可能持续很久 | 页面不能随着记录增加越来越慢 |

## 一句话任务

> 盘点员在仓库货架前使用 PDA，为了完成当前盘点任务，需要先确认库位和物料是否匹配，再录入实盘数量并提交；成功后自动进入下一次扫码，失败时保留当前记录并给出可恢复动作。
