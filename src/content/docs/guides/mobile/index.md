...
description: 面向活字格 PDA 移动端页面的设计规范，覆盖任务定义、信息架构、交互行为、视觉可读性、性能与稳定性。
sidebar:
  order: 1
---
...
| 真机验证 | 建议在目标 PDA、HAC 或目标 WebView 中确认布局、扫码、弱网和性能。 |
...
| 5. 保障稳定 | [性能与稳定性](./performance/) | 首屏加载、弱网离线、提交超时、旧 WebView 兼容。 |

最终交付物不应只是一张页面截图，而应是一份可以落地实施的页面设计说明。说明中需要写清楚页面为什么存在、现场人员如何完成任务，以及扫码和异常如何恢复。
...
| 弱网丢数据、旧设备卡顿、真机表现不一致 | [性能与稳定性](./performance/) |
